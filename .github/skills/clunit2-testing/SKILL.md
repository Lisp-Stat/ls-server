...
[clunit2](https://codeberg.org/cage/clunit2) is a Common Lisp unit testing
framework. Package: `:clunit`.
...
When debugging failures, run individual tests or suites instead of the full
suite. All targeted execution requires rebinding `clunit:*test-output-stream*`
to see output (see Known Issue #1 below).

### Run a Single Test

```lisp
(let ((clunit:*test-output-stream* *standard-output*))
  (clunit:run-test 'my-test-name))
```

### Run a Single Suite (and its children)
...
```lisp
(let ((clunit:*test-output-stream* *standard-output*))
  (clunit:run-tests :suite-name 'my-suite :tests '(test-a test-b)))
```

### CLI Invocation

See `.github/copilot-instructions.md` for the SBCL one-liner for your project.
Replace `SUITE-NAME` with the suite or test name you want to isolate.
...
```lisp
;;; NOTE: Uses internal function from my-package — update if API changes.
(defun call-internal-fn (&rest args)
  (apply #'my-package::internal-fn args))
```
